...
function getEmbedInfo(rawUrl) {
  ...
  if (host === 'vimeo.com' || host.endsWith('.vimeo.com')) {
    const id = path.replace(/^\//, '').split('/')[0];
    if (/^\d+$/.test(id)) return iframeEmbed(
      `https://player.vimeo.com/video/${id}`,
      `?autoplay=1`,
      `https://vumbnail.com/${id}.jpg`,    // proxy free de vimeo thumbs
      'vimeo'
    );
  }
  ...
  if (host === 'tiktok.com' || host.endsWith('.tiktok.com')) {
    ...
    if (m) return iframeEmbed(`https://www.tiktok.com/embed/v2/${m[1]}`, '', null, 'tiktok');
    return { kind: 'link', embedUrl: rawUrl, autoplayUrl: rawUrl, thumbnailUrl: null, platform: 'tiktok' };
  }

  // ---- Instagram ----
  if (host === 'instagram.com' || host.endsWith('.instagram.com')) {
    const m = path.match(/\/(reel|p|tv)\/([^\/]+)/);
    if (m) return iframeEmbed(`https://www.instagram.com/${m[1]}/${m[2]}/embed/`, '', null, 'instagram');
  }

  // ---- Facebook ----
  if (host === 'facebook.com' || host === 'fb.watch') {
    return iframeEmbed(`https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(rawUrl)}&show_text=false`, '&autoplay=1', null, 'facebook');
  }

  // ---- Spotify ----
  if (host === 'open.spotify.com' || host === 'spotify.com') {
    const m = path.match(/\/(track|episode|playlist|album)\/([^\/?]+)/);
    if (m) return iframeEmbed(`https://open.spotify.com/embed/${m[1]}/${m[2]}`, '', null, 'spotify');
  }

  // ---- SoundCloud ----
  if (host === 'soundcloud.com' || host === 'on.soundcloud.com') {
    const widgetUrl = `https://w.soundcloud.com/player/?url=${encodeURIComponent(rawUrl)}&color=%23ff5500&visual=true`;
    return iframeEmbed(widgetUrl, '&auto_play=true', null, 'soundcloud');
  }

  // ---- Archivos directos ----
  if (/\.(mp3|wav|ogg|m4a|flac)(\?.*)?$/i.test(path)) {
    return { kind: 'audio', embedUrl: rawUrl, autoplayUrl: rawUrl, thumbnailUrl: null, platform: 'audio' };
  }
  if (/\.(mp4|webm|mov|mkv)(\?.*)?$/i.test(path)) {
    return { kind: 'video', embedUrl: rawUrl, autoplayUrl: rawUrl, thumbnailUrl: null, platform: 'video' };
  }

  // ---- Google Drive ----
  if (host === 'drive.google.com') {
    const m = path.match(/\/file\/d\/([^\/]+)/);
    if (m) return iframeEmbed(`https://drive.google.com/file/d/${m[1]}/preview`, '', null, 'drive');
  }

  // Fallback: link clickeable, no reproducible
  return { kind: 'link', embedUrl: rawUrl, autoplayUrl: rawUrl, thumbnailUrl: null, platform: 'link' };
}

function iframeEmbed(baseUrl, autoplayQs, thumbnailUrl, platform) {
  return {
    kind: 'iframe',
    embedUrl: baseUrl,
...
/**
 * Intenta sacar un thumbnail real via noembed.com (proxy oEmbed gratuito
 * que funciona para TikTok, IG, Spotify, SoundCloud, Vimeo, YouTube,
 * Twitch, Reddit, etc.). Devuelve el URL del thumbnail o null.
 *
 * Tiene timeout corto (3s) — si noembed esta caido, no bloqueamos al
 * usuario que esta enviando su inscripcion. El fallback es el thumbnail
 * directo (YouTube/Vimeo) o el icono de plataforma.
 */
async function tryFetchOembedThumbnail(rawUrl) {
  if (!rawUrl) return null;
  try {
    ...
    const res = await fetch(`https://noembed.com/embed?url=${encodeURIComponent(rawUrl)}`, {
      signal: ctrl.signal,
      headers: { 'User-Agent': 'elpajaro/1.0' },
    });
    clearTimeout(timer);
    ...
    // timeout, network error, parse error — sin drama, fallback al directo.
    return null;
  }
}
 ...
 */
async function createSubmission({ country, name, instagram, mediaUrl, ip }) {
  ...
  // Pre-computamos el embed info al guardar para que listByCountry y el
  // modal de elim lo tengan listo sin tener que parsear el URL en cada
  // request. Cuando se llega al bracket, el contestant copia estos campos.
  const embed = getEmbedInfo(urlCheck.url) || { kind: 'link', embedUrl: urlCheck.url, autoplayUrl: urlCheck.url, thumbnailUrl: null, platform: 'link' };

  // Si la plataforma no tiene URL directo de thumbnail (TikTok, IG, Spotify,
  // SoundCloud, link generico), intentamos via noembed.com — proxy oEmbed
  // gratuito que devuelve el thumbnail oficial de la plataforma. Si falla
  // (timeout, plataforma no soportada), seguimos sin thumb (cae al fallback
  // gradient + icono).
  if (!embed.thumbnailUrl) {
    const oembedThumb = await tryFetchOembedThumbnail(urlCheck.url);
    if (oembedThumb) embed.thumbnailUrl = oembedThumb;
  }
  ...
  const sub = {
    ...
    clipEmbed: embed.embedUrl,           // src para mostrar (no-autoplay)
    clipEmbedAutoplay: embed.autoplayUrl,// src para play
    clipThumbnail: embed.thumbnailUrl,   // miniatura (YouTube/Vimeo) o null
    clipPlatform: embed.platform,        // 'youtube'|'vimeo'|'tiktok'|...
    ip: cleanIp,
    status: 'pending',
